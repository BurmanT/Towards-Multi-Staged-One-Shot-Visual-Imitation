...
class PickPlaceController:
    ...
    def reset(self): # reset function should also take in what object to manipulate
        #print("In reset function: object ids are ")
        #print(self._env.object_id)
        #self._object_name = [self._env.objects[i].name for i in self._env.object_id]
        #print("object names are ")
        #print(self._object_name)
        self._object_name = self._env.objects[self._env.object_id].name
        #print("In reset function: self._object_name is ")
        #print(self._object_name)
...
def get_expert_trajectory(env_type, controller_type, renderer=False, camera_obs=True, task=None, ret_env=False,
    ...
    print("Beginning trajectory collection")
    #print("Task ID is ")
    #print(task)
    #print("New id is ")
    second_id = 5
    ...
    while not success:
        ...
        #print("env.horizon is ... ")
        #print(int(env.horizon // 10))
        for t in range(int(env.horizon // 5)):
            ...
            if reward:
                ...
                if(not first_task_completed):
                    ...
                    print("FIRST task completed now second task")
                    env.change_task_id(second_id)
                    controller.reset()
                else:
                    success = True
                    print("SECOND TASK ALSO COMPLETED")
                    break
                '''
                if((not first_task_completed) and (not second_task_completed)):
                    first_task_completed = True 
                    print("FIRST task completed now second task")
                    env.change_task_id(second_id)
                    controller.reset()
                elif(first_task_completed and not second_task_completed):
                    print("SECOND task completed now third task")
                    second_task_completed = True
                    env.change_task_id(third_id)
                    controller.reset()
                else:
                    success = True
                    print("THIRD TASK ALSO COMPLETED")
                    break
